-- Fix: "column reference id is ambiguous" in diego_create_pos_order.
-- RETURNS TABLE(id, ...) creates PL/pgSQL variables that clash with table columns.

drop function if exists public.diego_create_pos_order(text, jsonb, text, uuid, text);

create function public.diego_create_pos_order(
  p_channel text,
  p_items jsonb,
...
returns table (order_id uuid, order_number bigint, order_total integer)
language plpgsql
security definer
...
begin
  ...
  returning "diego-orders".id into v_order_id;
  ...
  if p_restaurant_table_id is not null then
    update public."diego-restaurant-tables" t
    set status = 'occupee'
    where t.id = p_restaurant_table_id;
  end if;
